...
int main() {
    ...
    // Попытка создать FIFO с именем test.fifo в текущей директории
    ...
    if (proc1 < 0) {
        ...
        // Создание второго процесса. Следование условию.
        // Второй процесс не будет напрямую родственен первому (только через общего родителя)
        ...
        if (proc2 < 0) {
            ...
            // Внутри второго порождённого процесса. 
            // В этом процессе будет производиться запись
            
            execl("/home/arutiunian/comp_tech/5sem/answers/10/2nd.c", "2nd", NULL);
        }

    } else {
        // Внутри первого порождённого процесса. 
        // В этом процессе будет производиться чтение
